...
export function getStripeClient(): Stripe {
	if (!stripeClient) {
		// config() will throw if STRIPE_SECRET_KEY is not set
		// Using type assertion since config() throws for undefined values
...
export function verifyWebhookSignature(
	...
	try {
		...
		// config() will throw if STRIPE_WEBHOOK_SECRET is not set
		// Using type assertion since config() throws for undefined values
...
export function verifySyncSecret(secret: string): boolean {
	const expectedSecret = config("STRIPE_SYNC_SECRET");
